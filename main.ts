...
function summarizeBlock(rows: ReducedTrialRow[], blockId: string): { accuracy: number } {
  const blockRows = rows.filter((row) => row.block_id === blockId);
  if (blockRows.length === 0) {
    return { accuracy: 0 };
  }
  const correctTrials = blockRows.filter((row) => row.probe_hit === true);
  return {
    accuracy: correctTrials.length / blockRows.length
  };
}
...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000001-ax-cpt",
    task_name: "AX-CPT Task",
    task_description: "HTML preview aligned to the local psyflow AX-CPT procedure and parameters.",
    settings,
    subInfo,
    stimBank,
